...
public class OneDimensionalGrid {
	 ...
	 */
	public OneDimensionalGrid(int N) throws IOException, InvalidFormatException {
	
		final int pointscell = 1;
		/*final String XLSX_FILE_PATH = "../../_download/_multi_dim/" + Integer.toString(N) + 
									  "_1" + 
									  ".xlsx";*/  
	...
	public double getBoundary(int quantizer, char ul) {
		...
		case '+':
			if(quantizer == this.getQuantizerSize()) {
				...
				return (this.grid.get(quantizer) + this.grid.get(quantizer + 1))/2.0;
			}
		case '-':
			if(quantizer == 1) {
	...
	public static OneDimensionalGrid buildWithCostantValues(double value, int N) {
		HashMap<Integer, Double> grid = new HashMap<Integer, Double>();
		
		for(int i = 1; i <= N; i++) {
	    	grid.put(i,value);
	    			}   
		OneDimensionalGrid costant = new OneDimensionalGrid();
