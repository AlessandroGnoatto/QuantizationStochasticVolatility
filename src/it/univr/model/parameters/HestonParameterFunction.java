...
public class HestonParameterFunction implements ParameterFunctionInterface {
	...
	public RealVector getDriftValue(double[] stateVariable, double time) {
		this.drift.setEntry(0, (this.r - 0.5*Math.max(stateVariable[1],0.001)));
		this.drift.setEntry(1, this.k*(this.theta - Math.max(stateVariable[1],0.001)));
	...
	public Array2DRowRealMatrix getDiffusionValue(double[] stateVariable, double time) {
		this.diffusion.setEntry(0, 0, Math.sqrt(Math.max(stateVariable[1],0.001)));
		this.diffusion.setEntry(0, 1, 0);
		this.diffusion.setEntry(1, 0, Math.sqrt(Math.max(stateVariable[1],0.001))*this.sigma*this.rho);
		this.diffusion.setEntry(1, 1, Math.sqrt(Math.max(stateVariable[1],0.001))*this.sigma*
