...
import it.univr.normal.quantizer.OneDimensionalGrid;
	 ...
	 */
	public ThreeDimProductQuantizer(OneDimensionalGrid first,
			OneDimensionalGrid second,
			OneDimensionalGrid third) {
	     
		HashMap<Integer, OneDimensionalGrid> grids = new HashMap<Integer, OneDimensionalGrid>();
		grids.put(1, first);
		grids.put(2, second);
		grids.put(3, third);
	...
	public static ThreeDimProductQuantizer buildWithConstantValues(double value1,
		...
		OneDimensionalGrid first  = OneDimensionalGrid.buildWithCostantValues(value1, N1);
		OneDimensionalGrid second = OneDimensionalGrid.buildWithCostantValues(value2, N2);
		OneDimensionalGrid third  = OneDimensionalGrid.buildWithCostantValues(value3, N3);
	...
	/*
	 * public double[] get(int first, int second, int third) { for(Map.Entry<int[],
	 * double[]> element : this.set.entrySet()) { if(element.getKey()[0] == first &&
	 * element.getKey()[1] == second && element.getKey()[2] == third) { return
	 * element.getValue(); } } throw new
	 * IllegalArgumentException("Point not found"); }
	 */
	...
	public double[] get(int... point) {
	    for(Map.Entry<int[], double[]> element : this.set.entrySet()) {
	        if(element.getKey()[0] == point[0] && element.getKey()[1] == point[1] && element.getKey()[2] == point[2]) {
