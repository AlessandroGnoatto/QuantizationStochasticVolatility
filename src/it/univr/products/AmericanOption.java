		...
		for(int i = quantizedModel.getNumberOfTimeSteps(); i >=  1; i--) {
			...
			for(Map.Entry<double[], Double> quantizerk : quantizedModel.getProductQuantizerDistribution(i-1).entrySet()) {
				...
				if(i == 1) {
					...
					return finalValue*Math.exp(-quantizedModel.getDeltaT()*0.04);
					}
				...
				payoff = Math.max(payoff*Math.exp(-quantizedModel.getDeltaT()*0.04), Math.max(quantizerk.getKey()[0] - this.strike,0));
